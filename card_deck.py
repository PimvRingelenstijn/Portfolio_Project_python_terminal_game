...
# create csv file with card info
with open("card_information.csv", "w", newline="") as card_info_csv:
    writer = csv.writer(card_info_csv)
    writer.writerow(['Card', 'Point Value']) # set headers
    for card, value in card_dict.items():
        writer.writerow([card, value])